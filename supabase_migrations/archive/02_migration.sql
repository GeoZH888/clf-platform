...
-- ────────────────────────────────────────────────────────────────────
-- Step 2: Delete the 4 test users from jgw_invites (legacy auth path)
-- ────────────────────────────────────────────────────────────────────
-- Discovered: marco (and possibly others) exist in BOTH jgw_invites and
-- jgw_registrations. Login picks Path A (invites) first, so legacy invite
-- modules win. Need to clean up invites too for full deletion.
--
-- jgw_device_sessions has FK invite_id → jgw_invites. Must clean sessions
-- first before deleting invites.

-- 2a. Delete device sessions tied to these invites
DELETE FROM jgw_device_sessions
WHERE invite_id IN (
  SELECT id FROM jgw_invites
  WHERE username IN ('xiaomi', 'wenping', 'zhang', 'marco')
);

-- 2b. Delete the legacy invite rows
DELETE FROM jgw_invites
WHERE username IN ('xiaomi', 'wenping', 'zhang', 'marco');

-- ────────────────────────────────────────────────────────────────────
-- Step 3: Delete the 4 test users from jgw_registrations
-- ────────────────────────────────────────────────────────────────────
-- 3a. Clean up device sessions tied to these users (FK user_id → auth.users)
DELETE FROM jgw_device_sessions
WHERE user_id IN (
  SELECT approved_user_id FROM jgw_registrations
  WHERE username IN ('xiaomi', 'wenping', 'zhang', 'marco')
...
-- ────────────────────────────────────────────────────────────────────
-- Step 5: Create the new super_admin user
-- ────────────────────────────────────────────────────────────────────
...
INSERT INTO jgw_registrations (
  ...
  '6c4991d7-2c16-49cc-a6f3-8a9dc0c3ed02',  -- real auth.users UUID
...
-- ────────────────────────────────────────────────────────────────────
-- Step 6: Pre-populate clf_user_modules for the new super_admin
-- ────────────────────────────────────────────────────────────────────
