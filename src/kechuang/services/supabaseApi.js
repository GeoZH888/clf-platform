...
const api = {
  ...
  teacherApplications: {
    async getMyApplication(userId) {
      const { data, error } = await supabase
        .from('dwxz_teacher_applications')
        .select('*')
        .eq('user_id', userId)
        .order('created_at', { ascending: false })
        .limit(1)
  ...
  admin: {
    async getStats() {
      const [users, students, teachers, classes, homework, applications] = await Promise.all([
        supabase.from('dwxz_users_view').select('id', { count: 'exact' }),
        supabase.from('dwxz_users_view').select('id', { count: 'exact' }).eq('role', 'student'),
        supabase.from('dwxz_users_view').select('id', { count: 'exact' }).eq('role', 'teacher'),
        supabase.from('dwxz_classes').select('id', { count: 'exact' }).eq('is_active', true),
        supabase.from('dwxz_homework').select('id', { count: 'exact' }).eq('is_active', true),
        supabase.from('dwxz_teacher_applications').select('id', { count: 'exact' }).eq('status', 'pending')
      ]);
