...
const PointsAdminPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // 统计数据
      const { data: pointsData } = await supabase.from('dwxz_user_points').select('total_points, available_points');
      ...
      const { count: userCount } = await supabase.from('dwxz_user_points').select('*', { count: 'exact', head: true });
      const { count: pendingCount } = await supabase.from('dwxz_reward_redemptions').select('*', { count: 'exact', head: true }).eq('status', 'pending');

      setStats({ totalIssued, totalSpent, totalUsers: userCount || 0, pendingRedemptions: pendingCount || 0 });

      // 规则
      const { data: rulesData } = await supabase.from('dwxz_point_rules').select('*').order('points', { ascending: false });
      setRules(rulesData || []);

      // 奖品
      const { data: rewardsData } = await supabase.from('dwxz_rewards').select('*').order('points_required', { ascending: true });
      setRewards(rewardsData || []);

      // 兑换订单
      const { data: redemptionsData } = await supabase.from('dwxz_reward_redemptions')
  ...
  const handleSaveReward = async () => {
    try {
      if (rewardForm.id) {
        await supabase.from('dwxz_rewards').update(rewardForm).eq('id', rewardForm.id);
      } else {
        await supabase.from('dwxz_rewards').insert([{ ...rewardForm, is_active: true }]);
      }
  ...
  const handleDeleteReward = async (id) => {
    ...
    await supabase.from('dwxz_rewards').delete().eq('id', id);
    loadData();
  };
  ...
  const handleUpdateOrderStatus = async (id, status, trackingNumber = null) => {
    ...
    await supabase.from('dwxz_reward_redemptions').update(update).eq('id', id);
    setMessage({ type: 'success', text: t.success });
    loadData();
  };

  const toggleRuleStatus = async (rule) => {
    await supabase.from('dwxz_point_rules').update({ is_active: !rule.is_active }).eq('id', rule.id);
    loadData();
  };
