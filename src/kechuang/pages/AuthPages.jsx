  ...
  const handleChange = async () => {
    ...
    try {
      if (supabase && user) {
        ...
        await supabase.from('dwxz_users_view').update({ password_hash: simpleHash(newPassword), updated_at: new Date().toISOString() }).eq('id', user.id);
        setSuccess(t.success);
...
export const TeacherResetApproval = () => {
  ...
  const loadRequests = async () => {
    try {
      if (supabase) {
        const { data } = await supabase.from('dwxz_admin_password_resets').select('*, user:users(id, username, name)').eq('status', 'pending').order('created_at', { ascending: false });
        setRequests(data || []);
      }
  ...
  const handleAction = async (requestId, action) => {
    try {
      if (supabase) {
        ...
        await supabase.from('dwxz_admin_password_resets').update(updateData).eq('id', requestId);
        loadRequests();
      }
