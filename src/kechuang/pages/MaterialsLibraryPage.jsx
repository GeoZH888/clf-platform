...
const MaterialsLibraryPage = () => {
  ...
  const loadMaterials = async () => {
    ...
    setLoading(true);
    try {
      let query = supabase.from('dwxz_teaching_materials').select('*').order('created_at', { ascending: false });
  ...
  const handleUpload = async () => {
    try {
      await supabase.from('dwxz_teaching_materials').insert([{
        ...uploadForm,
  ...
  const handleDelete = async (id) => {
    ...
    await supabase.from('dwxz_teaching_materials').delete().eq('id', id);
    loadMaterials();
  };

  const handleApprove = async (id) => {
    await supabase.from('dwxz_teaching_materials').update({ is_reviewed: true }).eq('id', id);
    loadMaterials();
  };
