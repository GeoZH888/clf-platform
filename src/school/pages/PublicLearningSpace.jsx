    ...
    try {
      if (supabase) {
        const [materialsRes, questionsRes, postsRes] = await Promise.all([
          supabase.from('materials').select('*').eq('is_active', true).eq('share_type', 'public'),
          supabase.from('qa_questions').select('*, author:users(name, name_zh)').eq('is_active', true),
          supabase.from('public_posts').select('*, author:users(name, name_zh)').eq('is_active', true).order('created_at', { ascending: false })
        ]);
