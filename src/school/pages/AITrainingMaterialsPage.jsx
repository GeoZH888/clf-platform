...
const AITrainingMaterialsPage = () => {
  ...
  const handleAdd = async (e) => {
    e.preventDefault();
    try {
      await supabase.from('ai_training_materials').insert([{
        ...form,
  ...
  const handleReview = async (id, status) => {
    try {
      await supabase.from('ai_training_materials').update({
        status,
        reviewed_by: user?.id,
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await supabase.from('ai_training_materials').delete().eq('id', id);
      setMessage({ type: 'success', text: t.success });
      loadData();
    } catch (err) {
      setMessage({ type: 'error', text: t.failed });
