...
const HSKTestManagementPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      // Load AI config
      const { data: configData } = await supabase.from('rag_config').select('*').limit(1).single();
  ...
  const handleSavePaper = async () => {
    ...
    try {
      ...
      if (paperForm.id) {
        await supabase.from('hsk_test_papers').update(payload).eq('id', paperForm.id);
      } else {
        await supabase.from('hsk_test_papers').insert([payload]);
      }
  ...
  const handleSaveQuestion = async () => {
    ...
    try {
      ...
      if (questionForm.id) {
        await supabase.from('hsk_test_questions').update(payload).eq('id', questionForm.id);
      } else {
        await supabase.from('hsk_test_questions').insert([payload]);
      }
  ...
  const handleAudioUpload = async (files) => {
    ...
    for (const file of files) {
      try {
        ...
        await supabase.from('hsk_audio_files').insert([audioRecord]);
      } catch (err) {
  ...
  const handleDeletePaper = async (id) => {
    ...
    await supabase.from('hsk_test_papers').delete().eq('id', id);
    loadData();
  };
...
${textContent.substring(0, 15000)}`;
      ...
      if (audios && audios.length > 0) {
        ...
        await supabase.from('hsk_audio_files').insert(audioRecords);
      }
