...
export default function PandaStudio() {
  ...
  // Load emotions from clf_panda_emotions; seed with DEFAULT_EMOTIONS if empty.
  useEffect(() => {
    (async () => {
      const { data, error } = await supabase
        .from('clf_panda_emotions')
        .select('*')
        .order('order_idx', { ascending: true });
      if (error) { console.warn('[panda emotions]', error); setLoadingEm(false); return; }
      if (!data || data.length === 0) {
        const seed = DEFAULT_EMOTIONS.map((e, i) => ({
          id: e.id, label: e.label, prompt: e.prompt,
          color: e.color || '#888888', order_idx: i,
        }));
        const { data: inserted } = await supabase
          .from('clf_panda_emotions').insert(seed).select();
        setEmotions(inserted || seed);
      } else {
        setEmotions(data);
      }
      setLoadingEm(false);
  ...
  async function deleteEmotion(id) {
    ...
    const { error } = await supabase
      .from('clf_panda_emotions').delete().eq('id', id);
    if (error) { alert('删除失败: ' + error.message); return; }
    setEmotions(em => em.filter(e=>e.id!==id));
    // Also clean up generated image asset (if any)
