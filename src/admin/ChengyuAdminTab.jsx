...
export default function ChengyuAdminTab({ apiKeys }) {
  ...
  // Load idioms
  useEffect(() => {
    supabase.from('clf_chengyu').select('*').order('sort_order')
...
格式：[{"idiom":"...","pinyin":"...","meaning_zh":"...","meaning_en":"...","meaning_it":"...","story_zh":"...","example_zh":"...","difficulty":2,"theme":"${batchTheme}","hsk_level":${batchHsk}}]`;
      ...
      const { data: inserted, error } = await supabase.from('clf_chengyu')
      ...
      // Reload
      const { data } = await supabase.from('clf_chengyu').select('*').order('sort_order');
      setIdioms(data ?? []);
    } catch (e) {
      log(`✗ 生成失败: ${e.message}`);
...
Composition: square format, balanced, focal subject centered, soft natural lighting, period-appropriate ancient Chinese rural or village setting.`;
      ...
      await supabase.from('clf_chengyu')
  ...
  async function handleDelete(id) {
    ...
    await supabase.from('clf_chengyu').delete().eq('id', id);
