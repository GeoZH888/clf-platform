...
//
// Fixes baked in:
//   1. Supabase Storage rejects non-ASCII keys → wordFilename() builds ASCII
//      slugs from pinyin (tone marks stripped) + 4-char hash of word_zh.
//        红色 (hóng sè)  → word_hong_se_a3f2.png
//        是   (shì)      → word_shi_4e8b.png
//   2. Import path: '../lib/supabase' (matches IllustrationStudio + CLFWordsAdminTab).
//   3. handleGenerate mirrors IllustrationStudio.jsx response handling: checks
//      data.error first, then data.url, then enhancedPrompt fallback. Also
//      handles altUrl fields and base64 returns. Console-logs the full
//      response so failures are diagnosable from DevTools.
//   4. Dropped target_type/word_id from the gateway payload — character version
//      doesn't send them and they may have been confusing ai-gateway.
...
const STYLE_PRESETS = [
  ...
  { id: 'custom', label: '✏️ 自定义', prompt: null },
];
...
export default function WordIllustrationStudio({ words = [], onClose, onUpdate }) {
  ...
  const handleGenerate = async () => {
    if (!selectedWord) { setMsg('error', '请先选择词语'); return; }
    const prompt = buildPrompt();
    if (!prompt) { setMsg('error', '提示词为空'); return; }
    ...
    try {
      ...
      console.log('[ai-gateway response]', { status: res.status, data });
     ...
     const b64 = data.base64 || data.image || data.b64_json || data.images?.[0]?.base64;
      ...
      if (data.enhancedPrompt) {
        setMsg('info', `📝 Enhanced prompt (paste into ComfyUI/Midjourney):\n${data.enhancedPrompt}`);
        return;
      }

      throw new Error(`未知响应: ${JSON.stringify(data).slice(0, 200)}`);
    } catch (err) {
  ...
  return (
    <div style={S.root}>
      <div style={S.header}>
        <h2 style={{ margin: 0, fontSize: 18, color: '#2E7D32' }}>🎨 词语插图工作室</h2>
        {onClose && <button onClick={onClose} style={S.closeBtn} aria-label="Close">✕</button>}
      </div>
      ...
      <div style={S.body}>
        <section style={S.section}>
          ...
          {selectedWord && selectedWord.illustratable === false && (
            <div style={S.warnBanner}>⚠ 此词已标记为「不需要插图」。可在词语列表里修改。</div>
          )}
          {selectedWord && (
            <div style={S.hint}>📁 文件名: <code>{previewKey}</code></div>
          )}
        </section>
        ...
        <section style={S.section}>
          ...
          {styleId === 'custom' ? (
            ...
            <div style={S.promptPreview}><em>{previewPrompt}</em></div>
          ) : null}
        </section>
        ...
        {currentImageUrl && !generatedUrl && (
          <section style={S.preview}>
            ...
            <button type="button" onClick={handleClearImage} style={S.btnDanger}>
              🗑 删除插图
            </button>
          </section>
        )}
...
const S = {
  root: { background: '#fff', borderRadius: 12, boxShadow: '0 8px 32px rgba(0,0,0,.12)', maxWidth: 720, width: '100%', maxHeight: '90vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' },
  header: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid #eee' },
  closeBtn: { background: 'transparent', border: 'none', fontSize: 20, cursor: 'pointer', color: '#888' },
  body: { padding: 20, overflowY: 'auto', flex: 1 },
  section: { marginBottom: 18 },
  label: { display: 'block', fontWeight: 600, marginBottom: 6, color: '#A0522D', fontSize: 14 },
  select: { width: '100%', padding: '8px 10px', border: '1px solid #ddd', borderRadius: 6, fontSize: 14, boxSizing: 'border-box' },
  pills: { display: 'flex', flexWrap: 'wrap', gap: 8 },
  pill: { padding: '6px 14px', borderRadius: 20, border: '1px solid #ccc', background: '#fff', cursor: 'pointer', fontSize: 13, transition: 'all .15s' },
  pillActive: { padding: '6px 14px', borderRadius: 20, border: '1px solid #2E7D32', background: '#2E7D32', color: '#fff', cursor: 'pointer', fontSize: 13 },
  textarea: { width: '100%', padding: 10, border: '1px solid #ddd', borderRadius: 6, fontSize: 13, fontFamily: 'inherit', boxSizing: 'border-box', marginTop: 8, resize: 'vertical' },
  promptPreview: { background: '#FAF3E0', padding: 10, borderRadius: 6, marginTop: 8, fontSize: 12, color: '#666', lineHeight: 1.5 },
  hint: { marginTop: 6, fontSize: 11, color: '#888', fontFamily: 'monospace' },
  warnBanner: { marginTop: 8, padding: '8px 12px', background: '#FFF3CD', border: '1px solid #FFE69C', borderRadius: 6, fontSize: 13, color: '#664D03' },
  actions: { display: 'flex', alignItems: 'center', gap: 12, margin: '14px 0', flexWrap: 'wrap' },
  divider: { color: '#999', fontSize: 13 },
  btnPrimary: { padding: '10px 18px', background: '#2E7D32', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 500 },
  btnPrimaryDisabled: { padding: '10px 18px', background: '#999', color: '#fff', border: 'none', borderRadius: 8, cursor: 'not-allowed', fontSize: 14, fontWeight: 500 },
  btnSecondary: { padding: '10px 18px', background: '#fff', color: '#2E7D32', border: '2px solid #2E7D32', borderRadius: 8, cursor: 'pointer', fontSize: 14, fontWeight: 500, display: 'inline-block' },
  btnDanger: { padding: '8px 14px', background: '#fff', color: '#c62828', border: '1px solid #c62828', borderRadius: 6, cursor: 'pointer', fontSize: 13, marginTop: 8 },
  statusSuccess: { padding: '10px 12px', borderRadius: 6, marginBottom: 12, fontSize: 13, background: '#D4EDDA', color: '#155724' },
  statusError: { padding: '10px 12px', borderRadius: 6, marginBottom: 12, fontSize: 13, background: '#F8D7DA', color: '#721C24' },
  statusInfo: { padding: '10px 12px', borderRadius: 6, marginBottom: 12, fontSize: 13, background: '#D1ECF1', color: '#0C5460', whiteSpace: 'pre-wrap' },
  preview: { marginTop: 16, paddingTop: 16, borderTop: '1px solid #eee', display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'flex-start' },
  previewLabel: { color: '#888', fontSize: 13, margin: 0 },
  previewImg: { maxWidth: 280, width: '100%', borderRadius: 10, border: '1px solid #eee', boxShadow: '0 2px 8px rgba(0,0,0,.08)' },
};