...
const AITrainingCenterPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const { data: kbData } = await supabase.from('rag_knowledge_bases').select('*').eq('is_active', true);
      ...
      let query = supabase.from('training_materials').select('*, users:uploaded_by(name, name_zh)');
  ...
  const approveMaterials = async (ids) => {
    try {
      await supabase.from('training_materials').update({
        review_status: 'approved',
        reviewed_by: user?.id,
        reviewed_at: new Date().toISOString()
      }).in('id', ids);
  ...
  const updateKBStats = async (kbId) => {
    const { count: docCount } = await supabase.from('rag_documents')
      .select('*', { count: 'exact', head: true })
      .eq('knowledge_base_id', kbId)
      .eq('status', 'completed');

    const { count: chunkCount } = await supabase.from('rag_chunks')
      .select('*', { count: 'exact', head: true })
      .eq('knowledge_base_id', kbId);

    await supabase.from('rag_knowledge_bases').update({
