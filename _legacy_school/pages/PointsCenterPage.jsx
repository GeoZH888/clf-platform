...
const PointsCenterPage = () => {
  ...
  const txt = {
    zh: {
      ...
      teacherEarnRules: [
        { icon: '📚', action: '上传教学资料', points: '+20' },
        { icon: '✅', action: '资料审核通过', points: '+50' },
        { icon: '📜', action: '创建成语内容', points: '+30' },
        { icon: '🎬', action: '上传教学视频', points: '+50' },
        { icon: '🎮', action: '创建学习游戏', points: '+40' },
        { icon: '❓', action: '创建测验题', points: '+30' }
      ]
    },
    en: {
      title: '🎯 Points Center',
  ...
  const getLevelInfo = (points) => {
    const levels = [
      { level: 1, name: language === 'zh' ? '学习新手' : 'Beginner', icon: '🌱', min: 0 },
      { level: 2, name: language === 'zh' ? '初级学员' : 'Junior', icon: '🌿', min: 100 },
      { level: 3, name: language === 'zh' ? '中级学员' : 'Intermediate', icon: '🌳', min: 300 },
      { level: 4, name: language === 'zh' ? '高级学员' : 'Advanced', icon: '🌲', min: 600 },
      { level: 5, name: language === 'zh' ? '学习达人' : 'Expert', icon: '⭐', min: 1000 },
      { level: 6, name: language === 'zh' ? '学习大师' : 'Master', icon: '🌟', min: 2000 },
      { level: 7, name: language === 'zh' ? '学习传奇' : 'Legend', icon: '💫', min: 5000 },
      { level: 8, name: language === 'zh' ? '学习之神' : 'God', icon: '👑', min: 10000 }
    ];
        ...
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '1rem', marginBottom: '1.5rem' }}>
            <div className="card" style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>{t.streak} ({t.days})</div>
            </div>
            <div className="card" style={{ textAlign: 'center' }}>
              <div style={{ fontSize: '2rem' }}>📅</div>
