...
const ChineseCulturePage = () => {
    ...
    try {
      if (supabase) {
        const [c, v, k, s] = await Promise.all([
          supabase.from('chengyu').select('*').eq('is_active', true),
          supabase.from('culture_videos').select('*').eq('is_active', true),
          supabase.from('culture_knowledge').select('*').eq('is_active', true),
          supabase.from('culture_shares').select('*').order('created_at', { ascending: false })
        ]);
  ...
  const handleTemplateSubmit = async (e) => {
    ...
    try {
      ...
      if (supabase) {
        await supabase.from('culture_shares').insert([{
