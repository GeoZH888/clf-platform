...
const HSKPage = () => {
    ...
    try {
      if (supabase && user) {
        const [regsRes, progressRes] = await Promise.all([
          supabase.from('hsk_registrations').select('*').eq('student_id', user.id),
          supabase.from('hsk_practice').select('*, question:hsk_questions(*)').eq('student_id', user.id)
        ]);
  ...
  const handleAnswer = async (questionId, answer) => {
    ...
    try {
      ...
      await supabase.from('hsk_practice').insert([{
        student_id: user.id,
