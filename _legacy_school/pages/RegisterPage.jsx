...
const RegisterPage = () => {
  ...
  const loadSchools = async () => {
    if (!supabase) return;
    const { data } = await supabase.from('schools').select('id, name, name_zh, code').eq('is_active', true);
    setSchools(data || []);
  };

  const loadClasses = async (schoolId) => {
    if (!supabase) return;
    const { data } = await supabase.from('classes').select('id, name, hsk_level').eq('school_id', schoolId);
    setClasses(data || []);
  };
  ...
  const handleInvitationRegister = async () => {
    ...
    try {
      ...
      // 记录使用
      await supabase.from('invitation_code_usage').insert([{
