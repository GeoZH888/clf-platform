...
const ChengyuManagementPage = () => {
  ...
  useEffect(() => {
    const loadAIConfig = async () => {
      if (!supabase) return;
      const { data } = await supabase.from('rag_config').select('*').limit(1).single();
      setAiConfig(data);
    };
    loadAIConfig();
  }, [supabase]);
...
只返回JSON，不要其他文字。`;
  ...
  const loadData = async () => {
    ...
    try {
      if (supabase) {
        const { data } = await supabase.from('chengyu').select('*').order('created_at', { ascending: false });
        setChengyuList(data || []);
      } else {
        // Demo data
        setChengyuList([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        // Update
        if (supabase) {
          await supabase.from('chengyu').update(form).eq('id', editItem.id);
        }
        ...
        if (supabase) {
          const { data } = await supabase.from('chengyu').insert([form]).select();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (supabase) {
        await supabase.from('chengyu').delete().eq('id', id);
      }
