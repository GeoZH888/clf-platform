...
const ClassesPage = () => {
  ...
  const handleCreateClass = async (e) => {
    e.preventDefault();
    try {
      await supabase.from('classes').insert([{ ...classForm, teacher_id: user.id }]);
  ...
  const handleEditClass = async (e) => {
    e.preventDefault();
    try {
      await supabase.from('classes').update(classForm).eq('id', selectedClass);
  ...
  const handleJoinRequest = async (requestId, action) => {
    try {
      ...
      await supabase.from('class_join_requests').update({ status: action, reviewed_at: new Date().toISOString() }).eq('id', requestId);
      if (action === 'approved' && request) {
        await supabase.from('class_enrollments').insert([{ class_id: request.class_id, student_id: request.student_id, status: 'active' }]);
      }
      showMessage('success', t.success);
      loadData();
