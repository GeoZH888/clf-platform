...
const InvitationCodePage = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      let query = supabase.from('invitation_codes')
      ...
      // 加载班级
      let classQuery = supabase.from('classes').select('id, name, hsk_level');
      if (isTeacher) {
  ...
  const handleDisable = async (id) => {
    await supabase.from('invitation_codes').update({ is_active: false }).eq('id', id);
    loadData();
  };
