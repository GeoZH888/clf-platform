# patch_5cards.py
# Rewrite CommunityHome with 5 cards using pillar field.

import pathlib

CONTENT = '''// src/community/CommunityHome.jsx
...
p = pathlib.Path("src/community/CommunityHome.jsx")
...
chinese = len(re.findall(r"[\u4e00-\u9fff]+", back))
escape_pat = chr(92) + 'u'
escapes_count = 0
i = 0
hex_chars = set('0123456789abcdefABCDEF')
while i < len(back) - 5:
    if back[i] == chr(92) and back[i+1] == 'u':
        if all(c in hex_chars for c in back[i+2:i+6]):
            escapes_count += 1
            i += 6
            continue
    i += 1

print(f"Chinese groups: {chinese}")
print(f"Raw escapes: {escapes_count}")
print(f"openSection: {'OK' if 'openSection' in back else 'MISSING'}")
print(f"5 cards (HSK + game): {'OK' if 'HSK_LEVELS' in back and 'gameModules' in back else 'MISSING'}")
print(f"FEIYI: {'OK' if 'const FEIYI' in back else 'MISSING'}")
