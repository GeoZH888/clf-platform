# patch_phase_g1_schema.py
# Phase G.1 — Write SQL migration for the unified learning architecture.
#
# Creates:
#   - clf_atoms                       (atom registry across all content types)
#   - clf_user_learning_state         (per user x atom: state, mastery, SM-2)
#   - clf_attempts                    (individual practice events for recency-weighted score)
#   - clf_corpus                      (corpus registry)
#   - clf_corpus_chunks               (corpus content tree)
#   - clf_chunk_atoms                 (m:m chunks <-> atoms)
#   - clf_chunk_embeddings            (vector(1536) for similarity search)
#
# After running this script, MANUALLY run the SQL in Supabase Dashboard.
# This script does NOT execute SQL — only writes the file.
...
# Write SQL file
p_sql = ROOT / "db_migration_phase_g1_schema.sql"
data = SQL.encode("utf-16","surrogatepass").decode("utf-16").encode("utf-8")
p_sql.write_bytes(data)
print(f"[OK] wrote {p_sql.name} ({len(data)} bytes, {SQL.count(chr(10))} lines)")

# Sanity check
print()
print("=== SQL file summary ===")
import re
table_creates = re.findall(r'CREATE TABLE IF NOT EXISTS (\w+)', SQL)
print(f"Tables to create:  {len(table_creates)}")
for t in table_creates:
    print(f"  - {t}")
print()
extensions = re.findall(r'CREATE EXTENSION IF NOT EXISTS (\w+)', SQL)
print(f"Extensions:        {extensions}")
print()
indexes = re.findall(r'CREATE INDEX IF NOT EXISTS (\w+)', SQL)
print(f"Indexes:           {len(indexes)}")
print()
policies = re.findall(r'CREATE POLICY (\w+)', SQL)
...
print("=" * 60)
print()
print("1. Open Supabase Dashboard for project yqcojudvvjntaajnrilr")
print("2. Click 'SQL Editor' in left sidebar")
print("3. Click 'New query'")
print("4. Open the file in your editor:")
print(f"   {p_sql.absolute()}")
print("5. Copy ALL contents")
print("6. Paste into the Supabase SQL Editor")
print("7. Click 'Run' (bottom right)")
print()
print("Expected output: 'Success. No rows returned.'")
print()
print("If you get errors:")
print("  - 'extension vector does not exist'  ->  pgvector not enabled")
print("    Go to Database > Extensions, search 'vector', enable it. Re-run.")
print("  - 'column ... already exists'        ->  ignore, IF NOT EXISTS handles it")
print("  - 'permission denied'                ->  use service_role connection,")
print("                                          NOT anon. SQL Editor uses postgres")
print("                                          role by default - should work.")
print()
print("VERIFY after running:")
print("  Run this query in Supabase SQL Editor (paste fresh):")
print()
print("    SELECT table_name FROM information_schema.tables")
print("    WHERE table_schema = 'public'")
print("      AND table_name LIKE 'clf_%'")
print("    ORDER BY table_name;")
print()
print("  Should include: clf_atoms, clf_attempts, clf_chunk_atoms,")
print("  clf_chunk_embeddings, clf_corpus, clf_corpus_chunks,")
print("  clf_user_learning_state (plus existing tables)")
print()
print("After SQL runs successfully, NEXT SESSION (G.2) will:")
print("  - Seed atoms from existing clf_characters / clf_words / etc.")
print("  - Add a Netlify function for embedding chunks")
print("  - Test similarity search with a sample query")
