...
const MessagesPage = () => {
  ...
  const loadMessages = async () => {
    ...
    try {
      if (supabase && user) {
        let query = supabase.from('dwxz_messages').select('*, sender:users!messages_sender_id_fkey(id, name, name_zh, username, role)');
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      await supabase.from('dwxz_messages').insert(messagesToInsert);
  ...
  const markAsRead = async (id) => {
    try {
      await supabase.from('dwxz_messages').update({ is_read: true }).eq('id', id);
      loadMessages();
    } catch (error) {
