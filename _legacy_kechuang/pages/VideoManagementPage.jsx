...
const VideoManagementPage = () => {
  ...
  const loadData = async () => {
    ...
    try {
      if (supabase) {
        const { data } = await supabase.from('dwxz_culture_videos').select('*').order('created_at', { ascending: false });
        setVideos(data || []);
      } else {
        setVideos([
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editItem) {
        if (supabase) {
          await supabase.from('dwxz_culture_videos').update(form).eq('id', editItem.id);
        }
        ...
        if (supabase) {
          const { data } = await supabase.from('dwxz_culture_videos').insert([form]).select();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      if (supabase) {
        await supabase.from('dwxz_culture_videos').delete().eq('id', id);
      }
